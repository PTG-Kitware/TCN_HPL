...
class FramePoses:
    ...
    def __post_init__(self):
        ...
        assert self.score.shape[0] == self.joint_positions.shape[0]
        assert self.score.shape[0] == self.joint_scores.shape[0]
        assert self.joint_positions.shape[1] == self.joint_scores.shape[1]

    def __bool__(self):
        return bool(self.score.size)


@dataclass
class FrameData:
    object_detections: Optional[FrameObjectDetections]
